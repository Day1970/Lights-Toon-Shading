var app = (function() {
    ...
    var mPurple = createPhongMaterial({kd:[0.6,0.,0.8]}); 
    ...
    createModel("torus", fs, [1,1,1,1], [0,.75,0], [0,0,0,0], [1,1,1,1], mGreen);
    createModel("sphere", fs, [1,1,1,1], [-1.25,.5,0], [0,0,0,0], [.5,.5,.5], mPurple);
    createModel("sphere", fs, [1,1,1,1], [1.25,.5,0], [0,0,0,0], [.5,.5,.5], mCyan);
    ...
    function initEventHandler() {
        ...
        window.onkeydown = function(evt) {
            ...
            case ('A'):
                illumination.animateLights = !illumination.animateLights;
                break;
            case ('T'):
                useToon = !useToon;
                break;
            ...
            case 'L': 
                illumination.lightAngle += lightStep;
                if (illumination.lightAngle > Math.PI*2) illumination.lightAngle -= Math.PI*2;
                updateLightPositions();
                break;

            case 'R': 
                illumination.lightAngle -= lightStep;
                if (illumination.lightAngle < 0) illumination.lightAngle += Math.PI*2;
                updateLightPositions();
                break;
    ...
    function updateLightPositions() {
        var a = illumination.lightAngle;
        var r = illumination.lightRadius;
        var h = illumination.lightHeight;
    ...
    function tick(now) {
    var dt = (now - lastTime) / 1000.0; // seconds
    if (dt < 0) dt = 0;
    lastTime = now;

    if (illumination.animateLights) {
        illumination.lightAngle += illumination.lightSpeed * dt;
        if (illumination.lightAngle > Math.PI * 2) {
            illumination.lightAngle -= Math.PI * 2;
        }
